...
out.tex: doc.md template.tex references.bib
	pandoc -f markdown -t latex --template template.tex --biblatex --bibliography references.bib -M date:$(shell git log -1 --format="%cs") -o "$@" "$<"

docs/index.html: doc.md references.bib template.html docs/style.css
	pandoc -f markdown -t html5 --mathjax --toc --number-sections --citeproc --bibliography references.bib --template template.html -M date:$(shell git log -1 --format="%cs") -o "$@" "$<"
...
# to strip those out, which is why that flag is there.
	pandoc -f markdown -t epub2 --webtex="https://latex.codecogs.com/png.image?%5Cdpi%7B130%7D%5Cbg%7Bwhite%7D" --toc --number-sections --citeproc --bibliography references.bib --css epub.css --strip-comments=true -M date:$(shell git log -1 --format="%cs") -o "$@" "$<"
